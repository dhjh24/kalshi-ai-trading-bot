...
>
> 2026-04-28 Codex follow-up: landed the remaining quick-flip/live-refresh hardening gaps from the 2026-04-27 audit list. Quick Flip AI failures now degrade to the heuristic movement analyzer instead of zero-confidence blocking, recent trade stats are fetched once per evaluation and threaded through fallback analysis, `_passes_portfolio_enforcer()` now receives current position exposure and labels paper/shadow/live modes explicitly, and `_execute_single_quick_flip()` pre-checks existing open positions before inserting a new one. The live-trade internal refresh endpoint now normalizes/branches on the requested topic while preserving the backward-compatible `live-trade-decisions` SSE refresh behavior for missing, null, or unknown topics.
>
> 2026-04-29 Codex docs refresh: public docs now describe the current `2.0.0` app surface plus unreleased changes: `UnifiedTradingBot` is canonical, `paper_trader.py` / the legacy paper signal fallback / `src/clients/xai_client.py` are removed, Quick Flip is an opt-in unified-runtime lane with heuristic fallback and live opt-in, and the dashboard docs include the `live-trade-decisions` SSE topic plus the internal refresh hook. Older notes below are retained as historical context.
>
...
>
> Historical 2026-04-27 note: landed the W11 `UnifiedTradingBot` / `ModelRouter` cleanup and the W4 shadow-drift auto-pause hook. This note has been superseded by the 2026-04-28 and 2026-04-29 status above; `paper_trader.py` is now removed rather than merely deprecated.
>
...
- A **5-model AI ensemble** ([src/agents/](src/agents/), [src/jobs/decide.py](src/jobs/decide.py)) routed through `LLM_PROVIDER=auto|codex|openai|openrouter` ([src/config/settings.py:46-63](src/config/settings.py#L46-L63)).
...
7. **G7 — Cleanup.** Retire legacy paths; complete for `paper_trader.py`, `beast_mode_bot.py`, and `src/clients/xai_client.py`, with only dashboard compatibility shims retained.
...
### W11 — Legacy Cleanup  *(Status: complete as of 2026-04-28.)*

- `paper_trader.py` is removed. `python cli.py run --paper` is the canonical paper-trading runtime against live market data.
- The legacy paper signal-tracker fallback is removed. `src/paper/tracker.py` and `src/paper/dashboard.py` now read only unified runtime data from `trading_system.db`.
- Top-level `beast_mode_bot.py` is removed. `UnifiedTradingBot` in `src/runtime/unified_bot.py` is the canonical orchestrator; `python cli.py run --beast` remains as an aggressive settings override.
- `src/clients/xai_client.py` is removed. Provider usage and daily counter compatibility live in `src/clients/shared_types.py`, while production calls use `ModelRouter`.
- `scripts/beast_mode_dashboard.py` remains as a thin compatibility wrapper; `python cli.py dashboard` is the production dashboard path.
- Net cleanup delta: deleted `src/clients/xai_client.py`, `tests/test_xai_client.py`, `beast_mode_bot.py`, and `paper_trader.py`; created `src/runtime/unified_bot.py`, `src/runtime/__init__.py`, and `tests/test_shared_types.py`.
- **Done when:** files removed, README updated, and the relevant Python/server/web tests remain green.
