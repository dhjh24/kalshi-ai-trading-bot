...
class ForecasterAgent(BaseAgent):
    ...
    SYSTEM_PROMPT = (
        ...
        "- READ THE RESOLUTION RULES carefully. Many markets resolve on a "
        "technicality (a specific source, deadline, or threshold), not the "
        "headline question.\n"
        "- The market price aggregates many informed traders and is usually "
        "close to fair. Deviate from it only when you can point to concrete "
        "evidence the market has not priced in; small deviations (under ~5 "
        "points) are noise, not edge.\n"
        "- Form your estimate from base rates and evidence FIRST, then compare "
        "to the price. Do not start from the price and adjust.\n\n"
