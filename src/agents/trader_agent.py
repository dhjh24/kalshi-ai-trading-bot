...
class TraderAgent(BaseAgent):
    ...
    SYSTEM_PROMPT = (
        ...
        "2. EV AFTER FEES -- Kalshi's exact taker fee is 0.07 x P x (1-P) per "
        "   contract (P = price in dollars): ~1.75 cents at 50c, ~1.2 cents at "
        "   25c/75c. A limit order resting INSIDE the spread fills as maker at "
        "   0.0175 x P x (1-P) (~0.4 cents at mid) — pricing inside the spread "
        "   is worth roughly 1.3 cents of edge by itself. Expected value = "
        "   (team's fair probability x 100) - entry price - fees. Only trade "
        "   when clearly positive with margin for error; a 2-3 cent gross edge "
        "   taken at the ask is usually NOT tradable after fees.\n"
        "3. PROBABILITY != CONFIDENCE -- The team's fair probability is the "
        "   estimate of the outcome; your confidence is how much you trust that "
        "   estimate. Never substitute one for the other: a 90% fair "
        "   probability you barely trust is a SKIP, not a 90c valuation.\n"
        "4. RISK CHECK -- Does the risk manager approve? Respect position sizing "
        "   recommendations.\n"
        "5. PRICE SETTING -- Set a limit price that gives you edge, preferably "
        "   resting inside the spread (maker). Never chase the market.\n"
        "6. CONVICTION -- You need high conviction from multiple sources to act. "
        "   When in doubt, SKIP. Skipping costs nothing; bad trades compound.\n\n"
