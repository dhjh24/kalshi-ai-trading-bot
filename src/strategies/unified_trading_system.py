...
class TradingSystemConfig:
    ...
    # Quick flip strategy REMOVED â€” 0% win rate, -$208 in 12 trades.
...
class UnifiedAdvancedTradingSystem:
    """
    The Beast Mode Trading System ðŸš€
    ...
    def __init__(
        ...
        # ðŸš¨ DYNAMIC CAPITAL: Will be set by async_initialize() from actual Kalshi balance
    ...
    async def async_initialize(self):
        ...
        try:
            ...
            if active_positions:
                self.logger.info(f"ðŸ“Š Active positions: {len(active_positions)}")
                for pos in active_positions:
                    ticker = get_position_ticker(pos) or '?'
                    exposure = get_position_exposure_dollars(pos)
                    pnl = float(pos.get('realized_pnl_dollars', '0'))
                    self.logger.info(f"  ðŸ“Œ {ticker}: exposure=${exposure:.2f}, realized_pnl=${pnl:.2f}")
            ...
            if self.total_capital < 10:  # Minimum $10 to trade
                self.logger.warning(f"âš ï¸ Total capital too low: ${self.total_capital:.2f} - may limit trading")
        ...
        self.logger.info(
            f"ðŸŽ¯ CAPITAL ALLOCATION: Market Making=${self.market_making_capital:.2f}, "
        ...
        try:
            ...
            if cash_status['emergency_status']:
                ...
                if emergency_action.action_type == 'halt_trading':
                    ...
                    self.logger.warning(f"âš ï¸ Need to close {emergency_action.positions_to_close} positions for cash reserves")
            
            # Enforce position limits if needed (after cash check)
            if limits_status['status'] in ['OVER_LIMIT', 'WARNING']:
                self.logger.info(f"âš ï¸  Position limits enforcement needed: {limits_status['recommendations']}")
                enforcement_result = await limits_manager.enforce_position_limits()
                if enforcement_result['action'] == 'positions_closed':
                    self.logger.info(f"âœ… CLOSED {enforcement_result['positions_closed']} positions to meet limits")
            ...
            self.logger.info(
                f"ðŸŽ¯ Unified Strategy Complete: "
    ...
    async def _execute_market_making_strategy(self, markets: List[Market]) -> Dict:
        ...
        try:
            ...
            self.logger.info(
                f"âœ… Market Making: {results['orders_placed']} orders, "
    ...
    async def _execute_directional_trading_strategy(self, markets: List[Market]) -> PortfolioAllocation:
        ...
        try:
            ...
            self.logger.info(
                f"âœ… Directional Trading: {len(allocation.allocations)} positions, "
    ...
    async def _execute_quick_flip_strategy(self) -> Dict:
        ...
        try:
            self.logger.info(
                f"ðŸŽ¯ Executing Quick Flip Strategy with ${self.quick_flip_capital:.2f}"
            )
        ...
        try:
            ...
            for market_id, allocation_fraction in allocation.allocations.items():
                try:
                    ...
                    # ðŸš¨ ONLY SKIP if we already have a position on the EXACT same market_id AND side
                    existing_position = await self.db_manager.get_position_by_market_and_side(market_id, intended_side)
                    
                    if existing_position:
                        self.logger.info(f"â­ï¸ SKIPPING {market_id} {intended_side} - exact position already exists (likely from immediate trade)")
                        ...
                        if opposite_position:
                            self.logger.info(f"ðŸ“Š {market_id} - Adding {intended_side} position (already have {opposite_side})")
                        else:
                            self.logger.info(f"ðŸ“Š {market_id} - New {intended_side} position")
                    ...
                    if not can_add_position:
                        ...
                        self.logger.info(f"âš ï¸ Position size ${initial_position_value:.2f} exceeds limits, attempting to reduce...")
                        ...
                        for reduction_factor in [0.8, 0.6, 0.4, 0.2, 0.1]:
                            ...
                            if can_add_reduced:
                                ...
                                self.logger.info(f"âœ… Position size reduced to ${initial_position_value:.2f} to fit limits")
                                break
                        else:
                            ...
                            if current_positions >= limits_manager.max_positions:
                                self.logger.info(f"âŒ POSITION COUNT LIMIT: {current_positions}/{limits_manager.max_positions} positions - cannot add new position")
                                results['failed_executions'] += 1
                                continue
                            else:
                                self.logger.info(f"âŒ POSITION SIZE LIMIT: Even minimum size ${initial_position_value * 0.1:.2f} exceeds limits")
                                results['failed_executions'] += 1
                                continue
                    
                    position_value = initial_position_value
                    self.logger.info(f"âœ… POSITION LIMITS OK FOR ALLOCATION: ${position_value:.2f}")
                    ...
                    if not can_trade_reserves:
                        self.logger.info(f"âŒ CASH RESERVES BLOCK ALLOCATION: {market_id} - {reserves_reason}")
                        results['failed_executions'] += 1
                        continue
                    
                    self.logger.info(f"âœ… CASH RESERVES OK FOR ALLOCATION: {market_id}")
                    ...
                    if success:
                        ...
                        self.logger.info(f"âœ… Executed position: {market_id} {intended_side} {quantity} at {price:.3f}")
                    else:
                        results['failed_executions'] += 1
                        self.logger.error(f"âŒ Failed to execute position for {market_id}")
    ...
    async def _manage_risk_and_rebalance(self, results: TradingSystemResults):
        ...
        try:
            ...
            if risk_violations:
                self.logger.warning(f"âš ï¸  Risk violations detected: {risk_violations}")
            ...
            if time_since_rebalance.total_seconds() > (self.config.rebalance_frequency_hours * 3600):
                self.logger.info("ðŸ”„ Portfolio rebalancing triggered")
                # TODO: Implement rebalancing logic
                self.last_rebalance = datetime.now()
            
            # Performance monitoring
            if results.portfolio_sharpe_ratio < self.config.target_sharpe_ratio * 0.5:
                self.logger.warning(f"âš ï¸  Low Sharpe ratio: {results.portfolio_sharpe_ratio:.2f}")
            
            if results.capital_efficiency < 0.8:
                self.logger.warning(f"âš ï¸  Low capital efficiency: {results.capital_efficiency:.1%}")
    ...
    def get_system_performance_summary(self) -> Dict:
        ...
        try:
            ...
            # ðŸš¨ FIX: Check if market_maker is initialized (not None)
...
async def run_unified_trading_system(
    ...
    try:
        ...
        # ðŸš¨ CRITICAL: Initialize with dynamic balance from Kalshi
        ...
        # Log final summary
        logger.info(
            f"ðŸŽ¯ UNIFIED SYSTEM COMPLETE ðŸŽ¯\n"
            ...
            f"  â€¢ Max Drawdown: {results.max_portfolio_drawdown:.1%}\n"
            f"  â€¢ Diversification Ratio: {results.diversification_ratio:.2f}\n"
            f"\n"
            f"ðŸ’° STRATEGY BREAKDOWN:\n"
            ...
            f"\n"
            f"ðŸš€ SYSTEM STATUS: BEAST MODE ACTIVATED! ðŸš€"
        )
